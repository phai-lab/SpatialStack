...
def _compute_resized_shape(width, height, mode="crop", target_size=518):
    if mode == "pad":
        if width >= height:
...
def preprocess_pil_images(image_path_list, mode="crop", target_size=518):
    ...
    for image_path in image_path_list:
        img = _normalize_rgb_image(image_path)
        width, height = img.size
        new_width, new_height = _compute_resized_shape(width, height, mode=mode, target_size=target_size)
...
def load_and_preprocess_images(image_path_list, mode="crop", target_size=518):
    ...
    for img in preprocess_pil_images(image_path_list, mode=mode, target_size=target_size):
        img = to_tensor(img)
